...
function docProfileSubmit() {
	...
	if ((dPharmaRoute=='') || (dNMDRoute=='')){ $("#myerror_doctor_prof").html('Please entry NMD & Pharma route ');
	$("#wait_image_docProf").hide();}else{$("#wait_image_docProf").show();$("#myerror_doctor_prof").html('' )
...
if ((dPharmaRoute=='') || (dNMDRoute=='')){ $("#myerror_doctor_add").html('Please entry NMD & Pharma route ');$("#wait_image_docAdd").hide();
	$("#btn_submit_doc_add").show();}else{
	$("#myerror_doctor_add").html('' )
...
function confirmDoc(docid) {
  ...
  $.ajax(localStorage.report_url+'doc_info_confirm?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&docID='+docid,{
								...
								if (resultArray[0]=='FAILED'){
									...
									if ((new_doc==1) && (status=='ACTIVE')){$("#pendingType").html('<span style="color:#007900; font-size:18px">ADDITION</span>');}
									if ((new_doc==0) && (status=='ACTIVE')){$("#pendingType").html('<span style="color:#00F; font-size:18px">UPDATE</span>');}
...
function confirmDocSubmit() {
	 ...
	 if (localStorage.confirmDoc==''){
		...
		if ((pharma_route=='') || (nmd_route=='')){ $("#myerror_doctorCon_add").html('Please entry NMD & Pharma route ');}else{
			$("#myerror_doctorCon_add").html('' )
	  $("#wait_image_docConAdd").show();
